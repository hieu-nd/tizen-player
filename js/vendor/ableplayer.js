...
(function ($) {
  ...
  AblePlayer.prototype.createPopup = function (which) {
    ...
    $popup.on('keydown',function (e) {
      ...
      else if (e.which === 32 || e.which === 13 || event.which === 19 || event.which === 415) { // space or enter
        $('input:focus').click();        
      }
  ...
  window.AccessibleDialog = function(modalDiv, dialogRole, title, descDiv, closeButtonLabel, width, fullscreen, escapeHook) {
    ...
    if (!fullscreen) {
      ...
      closeButton.keydown(function (event) {
        // Space key down
        if (event.which === 32 || event.which === 19) {
          thisObj.hide();
        }
      }).click(function () {
        thisObj.hide();
...
(function ($) {
  ...
  AblePlayer.prototype.onPlayerKeyPress = function (e) {
    ...
    else if (which === 32 || which===19) { // spacebar = play/pause     
    ...
    else if (which === 112 || which === 415) { // p = play/pause        
    ...
    else if (which === 102 || which ===417) { // f = forward 
      if (this.usingModifierKeys(e)) { 
        this.handleFastForward();
      }
    }
    else if (which === 114 || which ===412) { // r = rewind (could use B for back???) 
      if (this.usingModifierKeys(e)) { 
        this.handleRewind();
      }
    }
...
(function ($) {
  ...
  AblePlayer.prototype.handleWindowButtonClick = function (e) { 
    ...
    if (e.which > 1) { 
      // user pressed a key 
      if (!(e.which === 32 || e.which === 13 || which === 19)) { 
